...
Description: Revenue & subscription health report in MainWP. Duplicate customers merged; ignore lists, notes, unlinking supported; hourly auto-refresh.
Version:     0.4.0
...
function spm_settings_page() {
	if ( isset( $_POST['spm_save_settings'] ) ) {
		check_admin_referer( 'spm_settings', 'spm_nonce' );
		update_option( 'spm_stripe_secret_key', sanitize_text_field( $_POST['spm_secret_key'] ) );
		delete_transient( 'spm_cached_report' );  // force rebuild with new key
...
function spm_dashboard_page() {
	...
	/* fetch data (force refresh if nonce present) */
...
function spm_get_cached_report( string $secret, bool $force=false ){
	...
	try{
		...
		/* customers */
		$raw=iterator_to_array($stripe->customers->all(['limit'=>100,'expand'=>['data.subscriptions']])->autoPagingIterator());
		$merged=[];
		foreach($raw as $cu){
			$alias=strtolower(trim($cu->email?:$cu->name));
			if(!$alias) $alias=$cu->id;
			if(!isset($merged[$alias])){$merged[$alias]=$cu;continue;}
			$merged[$alias]->subscriptions->data=array_merge($merged[$alias]->subscriptions->data,$cu->subscriptions->data);
		}
		$raw=array_values($merged);

		/* charges for totals & last_paid */
		$life=$last=[];
		foreach($stripe->charges->all(['limit'=>100])->autoPagingIterator() as $ch){
			if($ch->status!=='succeeded'||!$ch->paid)continue;
			$e=strtolower(trim($ch->billing_details->email?:'')); if(!$e)continue;
			$life[$e]=($life[$e]??0)+$ch->amount;
			$last[$e]=max($last[$e]??0,$ch->created);
		}
		...
		foreach($stripe->invoices->all(['limit'=>100,'status'=>'open'])->autoPagingIterator() as $inv){
			...
			if($due && $due<time()){
				$e=strtolower(trim($inv->customer_email?:'')); if($e) $overdue_ids[$e]=true;
			}
		}
		...
		foreach($raw as $cu){
			$alias=strtolower(trim($cu->email?:$cu->name)); if(!$alias) $alias=$cu->id;
			$mrr=0;
			...
			if((time()-$paid)/DAY_IN_SECONDS>SPM_OVERDUE_DAYS) $overdue_ids[$alias]=true;
			$customers[$alias]=[
...
register_activation_hook(__FILE__,'spm_schedule_cron');
